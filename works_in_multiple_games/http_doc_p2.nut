...
::ProcessGET <- function() {
    ...
    printl("")
    for (local i = 0; i <= str.len() / 128; i ++) {
        print(str.slice(i * 128, min(i * 128 + 128, str.len())));
    }
    // for good measure
    printl("")
    printl("")
...
// Original script from Strata Source, modified for ~~Titanfall~~ All Portal 2 branch games
